  ...
  class Scraper
    ...
    def self.member_scrapers
      ...
      results = [klobuchar(year), kilmer, lacyclay, sullivan, halrogers, shaheen, timscott, wenstrup, bucshon, angusking, document_query_new, fulcher, gardner, jordan, watkins, lamborn,
        ...
        bennet(page=1), drupal, durbin(page=1), gillibrand, kennedy, duckworth, senate_drupal_newscontent, senate_drupal, vandrew, mcbath, blunt, tillis, coons, hayes, barr, emmer, porter].flatten
    ...
    def self.porter(page=1)
      ...
      return if doc.nil?
      doc.xpath("//article").each do |row|
        results << {:source => url, :url => "https://porter.house.gov" + row.css("h3 a").first['href'], :title => row.css("h3").text.strip, :date => Date.parse(row.css('time').first['datetime']), :domain => domain }
      end
      results
    end
